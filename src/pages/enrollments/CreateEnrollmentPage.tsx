...
const CreateEnrollmentPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.response?.data?.message ?? 'Failed to create enrollment. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="relative w-full mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-6">
...
/>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Create Enrollment</h1>
      ...
      <Card className="w-full max-w-full md:max-w-2xl mx-auto shadow-sm">
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">Enrollment Details</CardTitle>
        </CardHeader>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {errors.student_id && <p className="text-red-600 text-sm mt-1">Please select a student</p>}
            </div>
            ...
            <div>
                  ...
                  {subjects.map((subject) => (
                    <SelectItem key={subject.id} value={subject.id.toString()} className="py-3">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-gray-500">
                          Class: {subject.class?.name ?? "N/A"} • Board: {subject.board?.name ?? "N/A"} {subject.is_course && "• Course"}
                        </span>
                      </div>
                    </SelectItem>
                  ))}
              ...
              {errors.subject_id && <p className="text-red-600 text-sm mt-1">Please select a subject</p>}
            </div>
