...
import { Plus, Video, MapPin, Clock, Calendar, Users, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function ClassSessionsPage() {
  ...
  const renderSessionCard = (session: ClassSession) => {
    ...
    return (
      ...
      >
        {/* Header */}
        <div className={`relative ${session.mode === 'ONLINE' ? 'bg-saBlueLight/60' : 'bg-amber-100'} text-gray-900 p-4 sm:p-5`}>
        ...
        <CardContent className="bg-gray-100 rounded-b-2xl pt-6 p-4 sm:p-5 space-y-3 text-gray-700 flex-1 flex flex-col justify-between">
          <div className="grid grid-cols-2 gap-2 sm:gap-3 text-xs sm:text-sm">
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-saBlue" />
              <span>{formatDate(session.start_time)}</span>
            </div>
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4 text-saBlue" />
